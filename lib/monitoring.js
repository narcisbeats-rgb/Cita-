...
import { monitorHours, monitorScheduleTimeZone, nextScheduledCheckAt } from './monitor-schedule.js';

export { monitorHours, monitorScheduleTimeZone };
export const monitorIntervalMinutes = 60;
export const cleanupRetentionDays = Math.max(0, Number(process.env.DATA_RETENTION_DAYS || 7));
let running = false;
let lastResult = null;
const lastResultsByService = {};
let lastAlert = null;
let nextMonitorAt = nextScheduledCheckAt();
let lastCleanupAt = 0;
...
async function runServiceMonitor(serviceKey, trigger, rows) {
  ...
  console.log(`[Detector de Citas] Result ${serviceKey}: ${result.state}${result.locationMatchType ? ` · location=${result.locationMatchType}` : ''}`);
...
export async function schedulerTick() {
  ...
  const scheduledSlot = nextMonitorAt;
  nextMonitorAt = nextScheduledCheckAt(scheduledSlot);
  await runMonitor('scheduled');
}
export function monitorState() {
  return {
    ...
    lastAlert,
    nextCheckAt: new Date(nextMonitorAt).toISOString(),
    monitorHours,
    scheduleTimeZone: monitorScheduleTimeZone
  };
}
