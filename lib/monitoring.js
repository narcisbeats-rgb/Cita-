...
import { activeSubscribers, dbConfigured, dbRequest, madridTimeZone, monitorableServices, serviceLabel, serviceSmsLabel, unpackSmsNumber } from './app-store.js';
    ...
    try {
      ...
      if (phone) {
        ...
        const text = result.state === 'AVAILABILITY_DETECTED'
          ? `Detector de Citas: cita detectada para ${serviceSmsLabel(serviceKey)}. Entra ahora y finaliza: ${link}`
...
async function runServiceMonitor(serviceKey, trigger, rows) {
  let result;
  try { result = await checkMadridTieAvailability({ safeMode: true, serviceKey }); }
  catch (error) { result = { ok: false, state: 'ERROR', startedAt: new Date().toISOString(), finishedAt: new Date().toISOString(), message: error?.message || String(error) }; }
  result.trigger = trigger;
  result.serviceKey = serviceKey;
  lastResult = result;
  lastResultsByService[serviceKey] = result;
  await recordChecks(serviceKey, rows).catch(() => {});
  if (['HUMAN_GATE', 'AVAILABILITY_DETECTED'].includes(result.state)) await notifySubscribers(result, serviceKey, rows).catch(() => {});
  return result;
}
