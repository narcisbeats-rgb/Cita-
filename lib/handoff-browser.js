...
async function classifyAvailability(page, session) {
  ...
  if (await detectHumanGate(page)) {
    ...
    setSessionState(session, 'AVAILABILITY_DETECTED', 'Se detectó una pantalla de disponibilidad. Revisa y confirma la cita.', 'MANUAL');
  } else {
...
export async function attemptWithBrowserHandoff({ ownerId, serviceKey, client, timeoutMs = 20000 }) {
  ...
  try {
    ...
    if (anyMatch(text, NO_AVAILABILITY_PATTERNS)) {
      result.ok = true;
      return await finish('NO_AVAILABILITY', 'Portalul indică faptul că nu există cita disponibilă acum.', 'MANUAL');
    }

    if (anyMatch(text, AVAILABILITY_PATTERNS)) {
      result.ok = true;
      return await finish('AVAILABILITY_DETECTED', 'Am detectat o pagină de disponibilitate. Revizuiește și confirmă cita.', 'MANUAL');
    }

    return await finish('READY_FOR_HUMAN_CONTINUE', 'Am ajuns la următorul pas al portalului. Revizuiește-l manual în CitaNIE.', 'MANUAL');
  } catch {
