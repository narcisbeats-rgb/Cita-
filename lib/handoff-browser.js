...
import { normalizeProcedureText, procedureDefinition } from './procedures.js';
...
const normalize = normalizeProcedureText;
...
async function selectMadrid(page) {
  try {
    ...
    if (currentUrl.pathname.includes('/icpplustiem/citar') && currentUrl.searchParams.get('p') === '28') {
      return { optionText: 'Madrid', direct: true };
    }
  } catch {}

  const prepared = await page.locator('input, select').evaluateAll((controls) => controls.some((control) => {
  ...
  if (prepared) return { optionText: 'Madrid', direct: true };
...
async function findProcedure(page, serviceKey, selectIt = true) {
  const patterns = procedureDefinition(serviceKey)?.patterns;
...
async function chooseDocumentType(page, client) {
  ...
  const requested = normalize(client.documentType);
  ...
  for (let i = 0; i < count; i++) {
    ...
    const index = normalized.findIndex((t) => matchesType(t, wanted));
    if (index < 0) continue;
...
export async function attemptWithBrowserHandoff({ ownerId, serviceKey, client, viewport: requestedViewport, timeoutMs = 20000 }) {
  ...
  try {
    setSessionState(session, 'OPENING_PORTAL', 'Abriendo directamente el portal oficial de Madrid…', 'START');
    try {
    ...
    if (!province) {
      if (await detectHumanGate(page)) {
        return await finish('HUMAN_GATE', 'El portal solicita una verificación al abrir Madrid. Complétala aquí.', 'PROCEDURE');
      }
      return await finish('READY_FOR_HUMAN_CONTINUE', 'La página de Madrid no terminó de cargar. Continúa dentro de la sesión oficial.', 'PROCEDURE');
    }
    session.province = province.optionText;

    setSessionState(session, 'SELECTING_PROCEDURE', 'Madrid preparado. Buscando el trámite oficial exacto…', 'PROCEDURE');
