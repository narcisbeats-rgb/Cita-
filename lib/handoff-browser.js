...
async function waitAndSelectProcedure(page, serviceKey, timeoutMs = 9000) {
...
async function clickContinue(page) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (!/ACEPTAR|CONTINUAR|ENTRAR|SIGUIENTE|SOLICITAR/.test(label)) continue;
    await item.click({ timeout: 7000 }).catch(() => {});
...
async function resumePipeline(session) {
  ...
  try {
    ...
    setSessionState(session, 'ERROR', 'El portal oficial no respondió correctamente. Inténtalo de nuevo.', 'MANUAL');
  } finally {
...
export async function attemptWithBrowserHandoff({ ownerId, serviceKey, client, timeoutMs = 35000 }) {
  const key = String(ownerId);
  ...
  try {
    await page.goto(ICP_URL, { waitUntil: 'commit', timeout: timeoutMs });
    ...
    if (!province) {
      if (await detectHumanGate(page)) {
        await park('HUMAN_GATE', 'Completa la verificación para continuar.', 'PROCEDURE');
        return result;
      }
      result.state = 'PORTAL_CHANGED';
      result.message = 'No se encontró Madrid en el portal oficial.';
      return result;
    }
    result.province = province.optionText;

    // Current portal: province + procedure can live on the same page.
    // Legacy portal: a Continue/Accept button can still exist between both steps.
    let procedure = await waitAndSelectProcedure(page, serviceKey, 7000);
    if (!procedure) {
      const legacyContinue = await clickContinue(page);
      if (legacyContinue) procedure = await waitAndSelectProcedure(page, serviceKey, 7000);
    }
    ...
    if (!procedure) {
      ...
      result.message = 'Madrid está seleccionado, pero el portal no mostró un trámite compatible.';
      return result;
    }
    ...
    const continued = await clickContinue(page);
    if (!continued) {
      await park('READY_FOR_HUMAN_CONTINUE', 'Madrid y el trámite están seleccionados. Revisa el siguiente paso.', 'IDENTITY');
      return result;
    }
    result.url = page.url();
    ...
    if (identityPage) {
      ...
      if (!fill.documentFilled) {
        ...
        result.message = 'El portal pide tus datos, pero no se pudo identificar con seguridad el campo del documento.';
        return result;
      }
    ...
    result.state = 'ERROR';
    result.message = 'El portal oficial no respondió correctamente. Inténtalo de nuevo.';
    return result;
  } finally {
