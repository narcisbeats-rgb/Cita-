import { cleanCityPreferences } from './location-preferences.js';
import { MONITORABLE_PROCEDURE_KEYS, procedureDefinition } from './procedures.js';
...
export const monitorableServices = new Set(MONITORABLE_PROCEDURE_KEYS);
...
export function clientFromStoredProfile(profile = {}) {
  return {
    ...
    preferredCities: cleanCityPreferences(profile.preferredCities),
...
export function serviceLabel(key) {
  return procedureDefinition(key)?.label || 'tu trámite de Extranjería';
}
export function serviceSmsLabel(key) {
  return procedureDefinition(key)?.smsLabel || 'tu trámite de Extranjería';
}
